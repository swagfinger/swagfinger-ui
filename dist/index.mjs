...
const AccordionContainer = styled_components_browser_esm.div(_t || (_t = Accordion_`
  display: block;
`));
const Accordion = ({
  render,
  multiOpen: _multiOpen = true,
  startActiveItems: _startActiveItems = []
}) => {
  const [activeItems, setActiveItems] = (0,react.useState)(_startActiveItems); //set initial active items // activeItems holds item indexs to show
  const [allowMultiOpen, setAllowMultiOpen] = (0,react.useState)(_multiOpen);
  (0,react.useEffect)(() => {
    console.log('Accordion is rendered');
  });
  (0,react.useEffect)(() => {
    _startActiveItems.forEach(item => {
      activeItemsCheck(item);
    });
  }, [_startActiveItems]);
  (0,react.useEffect)(() => {
    setAllowMultiOpen(_multiOpen);
  }, [_multiOpen]);
  const activeItemsCheck = (0,react.useCallback)(index => {
...
;// CONCATENATED MODULE: ./src/components/accordion/AccordionItem.js
let AccordionItem_ = t => t,
  AccordionItem_t,
  _t2,
  _t3;
...
const AccordionItemContainer = styled_components_browser_esm.div(AccordionItem_t || (AccordionItem_t = AccordionItem_`
  display: block;
  margin-bottom: 5px;
`));
const AccordionItemTitle = styled_components_browser_esm.div(_t2 || (_t2 = AccordionItem_`
...
`));
const AccordionItemContent = styled_components_browser_esm.div(_t3 || (_t3 = AccordionItem_`
  padding: 15px;
  &.show {
    display: block;
    border: 1px solid red;
...
const AccordionItem = ({
  ...
  return /*#__PURE__*/react.createElement(AccordionItemContainer, null, /*#__PURE__*/react.createElement(AccordionItemTitle, {
    onClick: index => onClick(index),
    className: isOpen ? 'show' : 'hide'
  }, data.title), /*#__PURE__*/react.createElement(AccordionItemContent, {
    className: isOpen ? 'show' : 'hide'
  }, data.body));
};
;// CONCATENATED MODULE: ./src/components/accordion/AccordionExample.js



const AccordionExample = () => {
  const [data, setData] = (0,react.useState)([]); //savedData is when we are saving data, 'data' is presented data that is not updated, we 'setData' on init with useEffect()
  (0,react.useEffect)(() => {
    setData([{
      title: 'helloworld',
